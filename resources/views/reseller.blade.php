...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .list-row em { color: var(--muted); font-size: 12px; font-style: normal; text-align: right; } .payment-row-actions { display: flex; align-items: center; justify-content: flex-end; gap: 7px; flex-wrap: wrap; } .payment-row-actions .btn { min-height: 32px; padding: 0 10px; }
            ...
            <div class="dashboard">
                <div>
                    ...
                    <section class="panel">
                        ...
                        <div class="panel-body"><fieldset data-sale-control><form id="plan-form" class="form-stack"><input type="hidden" name="id"><div class="two-col"><label class="field">基础套餐<select id="plan-template-select" name="base_plan_id" required><option value="">请选择管理员已上架套餐</option></select></label><label class="field">展示名称<input name="name" placeholder="例如：高速月付" required></label></div><label class="field">展示内容<textarea name="content" placeholder="面向客户展示的套餐说明"></textarea></label><div class="two-col"><label class="field">月付（元）<input name="month_price" type="number" min="0.01" step="0.01" inputmode="decimal"></label><label class="field">季付（元）<input name="quarter_price" type="number" min="0.01" step="0.01" inputmode="decimal"></label><label class="field">半年付（元）<input name="half_year_price" type="number" min="0.01" step="0.01" inputmode="decimal"></label><label class="field">年付（元）<input name="year_price" type="number" min="0.01" step="0.01" inputmode="decimal"></label><label class="field">两年付（元）<input name="two_year_price" type="number" min="0.01" step="0.01" inputmode="decimal"></label><label class="field">三年付（元）<input name="three_year_price" type="number" min="0.01" step="0.01" inputmode="decimal"></label><label class="field">一次性（元）<input name="onetime_price" type="number" min="0.01" step="0.01" inputmode="decimal"></label></div><div class="form-actions"><button class="btn btn-primary" type="submit">保存销售套餐</button><span class="hint">至少设置一个大于 0 元的周期价格</span></div></form></fieldset><div id="plans" class="data-list" style="margin-top:15px"></div></div>
                    </section>
                </div>
                <div>
