...
(function () {
    ...
    // 25 条模板规则的「模式」在引擎（各语种共用），替换文由字典文件按 id 提供；
    ...
    var RULE_PATTERNS = [
        ...
        ['manual_progress', /^已扫描订阅 (\d+) \/ (\d+)，发现可疑 (\d+) 个（(\d+)%）$/],
        ['manual_done', /^共扫描 (\d+) 个订阅，窗口内有数据 (\d+) 个，命中规则 (\d+) 个。$/],
        ['manual_overflow', /^可疑结果超过 200 条上限，仅列出前 200 条，其余 (\d+) 条未展示；建议收窄评估窗口或调高规则阈值。$/]
    ];
